...
import OrbitalCanvas, { type OrbitalCanvasHandle, type PlanetData } from '../components/orbital/OrbitalCanvas'
...
const PlanetPanel = ({ planet, onClose }: { planet: PlanetData; onClose: () => void }) => (
  <div style={{
    position: 'absolute', top: '1.5rem', right: '1.5rem', zIndex: 20,
    width: '260px', background: 'rgba(7,7,26,0.92)',
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ width: '16px', height: '16px', borderRadius: '50%',
          background: planet.color, boxShadow: `0 0 10px ${planet.glowColor}`, flexShrink: 0 }} />
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1rem',
          fontWeight: 700, letterSpacing: '0.1em', color: 'var(--color-text)' }}>
          {planet.name.toUpperCase()}
        </h3>
      </div>
      <button onClick={onClose} style={{ background: 'none', border: 'none',
        color: 'var(--color-muted)', cursor: 'pointer', fontSize: '1.1rem', lineHeight: 1 }}>
        ×
      </button>
    </div>
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginBottom: '1rem' }}>
      {[
        ...
        { label: 'Período orbital', value: `${planet.orbitalPeriodDays.toLocaleString()} días` },
        { label: 'Diámetro', value: `${planet.diameterKm.toLocaleString()} km` },
        ...
        <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.8rem',
            color: planet.glowColor, fontWeight: 700 }}>
            {value}
          </span>
        </div>
      ))}
    </div>
    <div style={{ height: '1px', background: `${planet.glowColor}22`, marginBottom: '0.75rem' }} />
    ...
    <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 57px)', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: '1.5rem', left: '2rem', zIndex: 10, pointerEvents: 'none' }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem',
          fontWeight: 700, letterSpacing: '0.15em', marginBottom: '0.25rem',
          color: 'var(--color-text)', textShadow: '0 0 20px rgba(99,102,241,0.8)' }}>
          SIMULADOR ORBITAL
        </h1>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: 'var(--color-muted)' }}>
          Scroll para zoom · Drag para mover · Click en planeta · Doble click para resetear
        </p>
      </div>
      ...
      <div style={{
        ...
        {SPEEDS.map(({ label, value }) => (
          <button key={value} onClick={() => handleSpeed(value)} style={{
            padding: '4px 12px', borderRadius: '999px', cursor: 'pointer',
            fontFamily: 'var(--font-body)', fontSize: '0.75rem',
            background: speed === value ? 'var(--color-accent)' : 'transparent',
