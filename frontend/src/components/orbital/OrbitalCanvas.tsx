...
const PLANETS: PlanetData[] = [
  {
    ...
    moons: 0,
    rings: false,
    speed: (BASE_SPEED / Math.pow(0.39, 1.5)) * 0.39,
  ...
  },
  {
    ...
    moons: 0,
    rings: false,
    speed: (BASE_SPEED / Math.pow(0.72, 1.5)) * 0.72,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 2,
    rings: false,
    speed: (BASE_SPEED / Math.pow(1.52, 1.5)) * 1.52,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 95,
    rings: false,
    speed: (BASE_SPEED / Math.pow(5.2, 1.5)) * 5.2,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 146,
    rings: true,
    speed: (BASE_SPEED / Math.pow(9.58, 1.5)) * 9.58,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 28,
    rings: true,
    speed: (BASE_SPEED / Math.pow(19.22, 1.5)) * 19.22,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 16,
    rings: false,
    speed: (BASE_SPEED / Math.pow(30.05, 1.5)) * 30.05,
    angle: Math.random() * Math.PI * 2,
  ...
  },
  {
    ...
    moons: 5,
    rings: false,
    speed: (BASE_SPEED / Math.pow(39.48, 1.5)) * 39.48,
    angle: Math.random() * Math.PI * 2,
...
const buildNebula = (W: number, H: number): HTMLCanvasElement => {
  ...
  const clouds = [
    ...
    { x: W * 0.5, y: H * 0.08, r: W * 0.18, color: "rgba(59,130,246,0.04)" },
    { x: W * 0.05, y: H * 0.85, r: W * 0.2, color: "rgba(99,102,241,0.03)" },
  ...
  for (let i = 0; i < 300; i++) {
    ...
    nCtx.fillStyle = `rgba(255,255,255,${op})`;
    nCtx.fill();
  }
  for (let i = 0; i < 15; i++) {
    const sx = Math.random() * W,
    ...
    nCtx.fillStyle = sg;
    nCtx.beginPath();
    nCtx.arc(sx, sy, 3, 0, Math.PI * 2);
    nCtx.fill();
  }
  return nc;
};

const OrbitalCanvas = forwardRef<OrbitalCanvasHandle, Props>(
  ({ onPlanetClick, realScale = false }, ref) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const animRef = useRef<number>(0);
    const { bodies } = useOrbitalBodies();
    ...
    useEffect(() => {
      if (bodies && bodies.length > 0) {
        planetsRef.current = planetsRef.current.map((p) => {
          const backendBody = bodies.find((b) => b.id === p.id);
          if (!backendBody) return p;
          return { ...p, mass: backendBody.mass };
        });
      }
    }, [bodies]);
    ...
    useEffect(() => {
      ...
      const handleWheel = (e: WheelEvent) => {
        ...
        const newZoom = Math.min(
          MAX_ZOOM,
          Math.max(MIN_ZOOM, zoomRef.current * zoomFactor),
        );
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        const distToSun = Math.hypot(mx - cx, my - cy);
        if (distToSun < 30 * zoomRef.current) {
          found = "sun";
        } else {
          planetsRef.current.forEach((p) => {
            ...
            if (
              Math.hypot(mx - screenX, my - screenY) <
              p.radius * zoomRef.current * 2 + 6
            )
              found = p.id;
          });
        }
      ...
      const drawSun = (cx: number, cy: number, zoom: number) => {
        ...
        corona.addColorStop(0, "rgba(255,180,0,0.15)");
        ...
        glow.addColorStop(0, "#fff7aa");
        glow.addColorStop(0.4, "#FDB813");
        glow.addColorStop(0.8, "#ff6600");
        ...
        core.addColorStop(0.3, "#fff7aa");
        core.addColorStop(1, "#FDB813");
        ctx.beginPath();
      ...
      const drawTrail = (
        ...
        for (let i = 1; i < trail.length; i++) {
          ...
          const [x2, y2] = trail[i];
          if (Math.hypot(x2 - x1, y2 - y1) > planet.orbitRadius * zoom * 1.5) continue;
          const sx1 = cx + (x1 - cx);
          const sy1 = cy + (y1 - cy);
          const sx2 = cx + (x2 - cx);
          const sy2 = cy + (y2 - cy);
          ...
          ctx.moveTo(sx1, sy1);
          ctx.lineTo(sx2, sy2);
          ctx.strokeStyle = `${planet.glowColor}${Math.floor(alpha * 255)
            .toString(16)
            .padStart(2, "0")}`;
          ctx.lineWidth = 1.5;
          ctx.stroke();
      ...
      const drawPlanet = (
        ...
        if (isHovered) {
          ctx.beginPath();
          ctx.arc(sx, sy, r * scale + 8, 0, Math.PI * 2);
          ctx.strokeStyle = `${planet.glowColor}99`;
          ctx.lineWidth = 1.5;
          ctx.setLineDash([4, 3]);
          ctx.stroke();
          ctx.setLineDash([]);
        ...
        glow.addColorStop(0, `${planet.glowColor}${isHovered ? "60" : "40"}`);
        glow.addColorStop(1, "transparent");
        ctx.beginPath();
        ctx.arc(sx, sy, r * scale * 3, 0, Math.PI * 2);
        ctx.fillStyle = glow;
        ctx.fill();
        ...
        if (planet.rings) {
          ...
          ctx.scale(1, 0.3);
          ctx.beginPath();
          ctx.arc(0, 0, r * scale * 2.4, 0, Math.PI * 2);
          ctx.strokeStyle = "rgba(232,213,163,0.5)";
          ctx.lineWidth = 4 * zoom;
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(0, 0, r * scale * 2.0, 0, Math.PI * 2);
        ...
        body.addColorStop(0, planet.glowColor);
        ...
        const shadow = ctx.createRadialGradient(
          sx + Math.cos(angle) * r * scale * 0.5,
          sy + Math.sin(angle) * r * scale * 0.5,
          0,
          sx,
          sy,
          r * scale,
      ...
      const draw = () => {
        ...
        planetsRef.current.forEach((p) => {
          ctx.beginPath();
          ctx.arc(cx, cy, p.orbitRadius * zoom, 0, Math.PI * 2);
